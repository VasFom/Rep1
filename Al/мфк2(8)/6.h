...
template <typename T>
T CountMark(T* mas) {//вычисляет средний балл
	float sum = 0;
	for (int i = 0; i < 5; i++) {
		sum += mas[i];
	}
...
template <typename T>
void CopyData(STUDENT<T>* mas, int numOut, int numIn) {//копирует данные из структуры с 
	mas[numIn].name = mas[numOut].name;				//с номером numOut в структуру с номером 
													//numIn
